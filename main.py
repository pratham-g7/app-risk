...
def assess_risk():
    ...
    for ver_app, (app_title, packages) in zip(ver_apps, scraped_data.items()):
        ...
        for pkg_lst in packages:
            for pkg in pkg_lst:
                pkg_obj = App(pkg)
                score = risk.risk_score(name_sim(app_title, str(pkg_obj.metadata["title"])), icon_sim(ver_app.metadata["icon"], pkg_obj.metadata["icon"]), 0, 0)
                print(f"- Package: {pkg}, Risk Score: {round(score*100, 2)}%")
        print("\n")
